<template>
    <section class="flex flex-col gap-6">
        <div v-if="showLocationMeta" class="rounded-2xl border border-gray-100 bg-gray-50 p-4 text-sm text-gray-800">
            <div class="flex flex-wrap gap-4">
                ...
                <span v-if="capacityLabel" class="inline-flex items-center gap-2 font-semibold text-primary-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 text-primary-600">
        ...
        <div v-if="blog.latitude && blog.longitude" class="mt-8 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            >
                ...
                ></iframe>
                <div class="absolute inset-0 flex items-center justify-center bg-black/10 transition group-hover:bg-black/20">
                    <span class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-600 shadow-lg transition group-hover:scale-105">
                        Xem trên Google Maps
                    </span>
                </div>
            </a>
            <p v-if="blog.address" class="mt-4 flex items-start gap-2 text-sm text-gray-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mt-0.5 h-5 w-5 shrink-0 text-primary-500">
