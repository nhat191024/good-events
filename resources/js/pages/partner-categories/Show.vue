      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4 border rounded-2xl p-3 md:p-4 bg-white shadow-md">
        ...
        <div class="md:col-span-4">
          <div class="aspect-square w-full rounded-2xl bg-gray-50 ring-1 ring-gray-200 overflow-hidden flex items-center justify-center">
            <img :src="props.item.image || placeholderImg" :alt="props.item.name" class="w-3/4 h-3/4 object-contain" />
          </div>
          <div class="mt-2 flex items-center gap-2">
            <div v-for="n in 5" :key="n" class="w-10 h-10 rounded-md bg-gray-100 ring-1 ring-gray-200"></div>
          </div>
        </div>
          ...
          <ul class="mt-2 space-y-1.5 text-[14px] md:text-[15px] text-black/80">
            <li class="flex items-center gap-2">
              <svg viewBox="0 0 24 24" class="w-4 h-4">
                <path d="M12 21s7-5.33 7-11a7 7 0 1 0-14 0c0 5.67 7 11 7 11Z" fill="none" stroke="currentColor" stroke-width="1.6"/>
      ...
      <div class="bg-white min-h-[50rem]">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mx-4">
          ...
          >
            ...
            <div class="mt-2 text-sm font-medium text-gray-900 group-hover:text-[#ED3B50] line-clamp-1">{{ r.name }}</div>
            <div class="text-xs text-gray-500">
              {{
                (r.min_price && r.max_price)
                  ? (r.min_price===r.max_price ? (r.min_price.toLocaleString('vi-VN')+' đ') : (r.min_price.toLocaleString('vi-VN')+' - '+r.max_price.toLocaleString('vi-VN')+' đ'))
                  : ((r.min_price||r.max_price) ? ((r.min_price||r.max_price)!.toLocaleString('vi-VN')+' đ') : 'Liên hệ')
              }}
            </div>
          </Link>
        </div>
