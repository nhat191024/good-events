...
- Query: `search`, `page`, `side` (`client` or `partner`, optional)
- Response:
...
```json
{
    "threads": [
        {
            "id": 1,
            "subject": "Customer name - Event name",
            "is_unread": false,
            "code": "BILL001",
            ...
            "latest_message": {
                ...
                "attachments": null,
                "location": null,
                "preview_text": "Text shown in thread preview",
    ...
    "messages": [
        {
            ...
            "message": {
                ...
                "preview_text": "Text shown in previews",
...
- Agent note: render from `message.type`. For `image`, use `message.attachments[].url`; for `location`, use `message.location.latitude` and `message.location.longitude`. `body` may be `null` except text messages.
...
- Image body: send as `multipart/form-data`.
    - `type`: `image`
    - `body`: optional caption, max 5000 chars
    - `images[]`: required, 1-5 image files, each max 5MB
- Location JSON body:
...
- Errors: `403` when the authenticated user is not a participant; `422` for validation errors; `500` when saving fails.
